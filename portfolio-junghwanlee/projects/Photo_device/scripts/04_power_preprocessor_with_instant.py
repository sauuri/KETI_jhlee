...
def preprocessor(feasible_consumption=False, split_running_status=False):
    _refined = pd.read_csv(
        DATA_PATH / "Final_merged_df_2025_0205.csv",
        header=0,
        low_memory=False,
        index_col=0,
    )
    _refined = _refined.rename(columns={'Time': COL_TIME})
    ...
    if feasible_consumption:
        ...
        refined.loc[indexer_infeasible, COL_POWER] = int(0)
        refined[COL_POWER] = refined[COL_POWER].interpolate()
...
if __name__ == "__main__":
    ...
    processed.to_csv(DATA_PATH / "Processed_0212_data.csv", index=False, header=True)
    filtered.to_csv(DATA_PATH / "Operating_Filtered_0212_data.csv", index=False, header=True)
