...
def main():
    ...
    data_rf = data[COLS_TO_INSPECT].copy()
    data_rf.to_csv(DATA_PATH / "HN_data" / "HN_data_refined.csv", index=False, header=True)

    print(f"Memory consumption for RAW(Single Precision): {sys.getsizeof(data)*1e-6:.3f} bytes")
