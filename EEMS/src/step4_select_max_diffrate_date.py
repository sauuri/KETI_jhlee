...
base_dir = Path.cwd().parents[1] / "src" / "preprocessing"/ "preprocessing_results" / "active_power"


save_df_result = [f for f in base_dir.glob("*.csv") if 'save' in f.stem][0]
...
def select_max_diffrate_date(df: pd.DataFrame, motor1_name: str, motor2_name: str) -> pd.DataFrame:
    date_box = []
    combined_date_box = []
...
inner_result_df = select_max_diffrate_date(inner_df, 'P1730A', 'P1730B')
outer_EXT_result_df = select_max_diffrate_date(outer_EXT_df, 'P7412A_EXT', 'P7412B_EXT')
outer_MCC_result_df = select_max_diffrate_date(outer_MCC_df, 'P7412A_MCC', 'P7412B_MCC')
